...
import { RotateCcw, Sparkles, ChevronLeft, ChevronRight, Plus, Loader2, CheckCircle, Tag } from 'lucide-react';
...
const WatchForm = ({ watch, onSave, onCancel, onNext, onPrevious, hasNext, hasPrevious }: WatchFormProps) => {
  ...
  const brands = ['Rolex', 'Omega', 'TAG Heuer', 'Breitling', 'Patek Philippe', 'Audemars Piguet', 'Cartier', 'IWC', 'Panerai', 'Tudor', 'Seiko'];
  ...
  useEffect(() => {
    if (formData.acquisitionCost) {
      ...
      if (selectedCurrency === 'EUR') {
        ...
        const convertedValue = (eurValue * exchangeRates[selectedCurrency]).toFixed(2);
  ...
  useEffect(() => {
    if (originalData) {
      ...
      const hasDataChanged = JSON.stringify(currentComparable) !== JSON.stringify(originalData);
  ...
  const handleResetAI = () => {
    if (window.confirm('Are you sure you want to reset the AI instructions? This action cannot be undone.')) {
  ...
  const handleGenerateDescription = async () => {
    ...
    console.log('Using images marked for AI:', formData.images.filter(img => img.useForAI));
    
    // Simulate API call with timeout
    setTimeout(() => {
      alert('Description generation would be implemented here using the AI instructions and selected images.');
  ...
  const handleCurrencyConversion = (newValue: string) => {
    ...
    if (selectedCurrency === 'EUR') {
      ...
      const eurValue = parseFloat(newValue) / exchangeRates[selectedCurrency];
  ...
  const handleEditBrands = () => {
    ...
    if (newBrand && newBrand.trim()) {
      ...
      alert(`Brand "${newBrand.trim()}" would be added to the list. This would be implemented with a proper modal in the real application.`);
    }
  };
  ...
  const handleEditLocations = () => {
    ...
    if (newLocation && newLocation.trim()) {
      ...
      alert(`Location "${newLocation.trim()}" would be added to the list. This would be implemented with a proper modal in the real application.`);
    }
  };
  ...
  const handleEditBatches = () => {
    ...
    if (newBatch && newBatch.trim()) {
      ...
      alert(`Batch group "${newBatch.trim()}" would be added to the list. This would be implemented with a proper modal in the real application.`);
    }
  };
  ...
  const aiSelectedCount = formData.images.filter(img => img.useForAI).length;
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        ...
        {hasPrevious && (
          <Button
            ...
            className="absolute left-8 top-1/2 transform -translate-y-1/2 z-10 bg-white shadow-lg hover:bg-gray-50"
          >
        ...
        {hasNext && (
          <Button
            ...
            className="absolute right-8 top-1/2 transform -translate-y-1/2 z-10 bg-white shadow-lg hover:bg-gray-50"
          >
            <ChevronRight className="h-4 w-4" />
          </Button>
        )}

        <div ref={formRef} tabIndex={-1} className="bg-white rounded-xl shadow-xl max-w-[90vw] w-full max-h-[98vh] flex flex-col overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      value={formData.sku}
                      readOnly
                      className="flex-1 px-3 py-2 border border-slate-300 rounded-lg bg-slate-50 text-slate-600 cursor-not-allowed"
                    />
                    <Button
                      type="button"
                      variant="outline"
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      placeholder="0.00"
                      className="w-36 px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm"
                    />
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-2.5">
                <div>
                  ...
                  >
                    ...
                    <option value="Platform Review">Platform Review</option>
                    <option value="Ready for listing">Ready for listing</option>
                    <option value="Listed">Listed</option>
                    <option value="Reserved">Reserved</option>
                    <option value="Sold">Sold</option>
                    <option value="Defect/Problem">Defect/Problem</option>
                ...
                <div>
                  ...
                  <div className="w-full px-3 py-2 border border-slate-300 rounded-lg bg-slate-50 text-slate-900 font-medium">
                    {formData.acquisitionCost ? `€${parseFloat(formData.acquisitionCost).toFixed(2)}` : '€0.00'}
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex-1 flex flex-col min-h-0">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center space-x-4 text-sm text-slate-600">
                        <span>{formData.images.length}/40 images</span>
                ...
                <div className="space-y-2.5 flex flex-col min-h-0">
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        className="w-full px-2 py-1 text-sm border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-amber-500 focus:border-transparent"
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      rows={1}
                      placeholder=""
                      className="w-full resize-y min-h-[40px]"
                    />
                    <div className="mt-3">
                      <Button
                        type="button"
                        ...
                        className="text-amber-600 hover:text-amber-700 hover:bg-amber-50 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                  ...
                  <div className="flex flex-col">
                    ...
                    <Textarea
                      ...
                      onChange={handleChange}
                      className="w-full resize-y min-h-[320px]"
                      disabled={isGeneratingDescription}
                    />
                  </div>
            ...
            <div className="flex gap-3 p-6 pt-4 border-t border-slate-200 flex-shrink-0">
              <Button
                type="button"
              ...
              <Button
                type="submit"
                className={`flex-1 ${!hasChanges ? 'bg-gray-400 text-gray-600 cursor-not-allowed' : ''}`}
                disabled={!hasChanges}
              >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-[60]">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <p className="text-gray-600 mb-6">
              You have unsaved changes. Do you want to save them before navigating to the next watch?
            </p>
            <div className="flex gap-3">
              <Button onClick={handleSaveAndNavigate} className="flex-1">
                Save & Continue
              </Button>
              <Button 
