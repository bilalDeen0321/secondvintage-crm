...
export const EditBatchModal = ({ isOpen, onClose, batch, availableWatches, batchWatchSortField, batchWatchSortDirection, onBatchWatchSort, onAddWatchToBatch, getSortedBatchWatches }: Props) => {
    ...
    const onWatchRemove = (routeKey: BatchResource["routekey"], watchKey: WatchResource["routeKey"]) => {
    ...
    const getSortIcon = (field: string) => {
        ...
        return batchWatchSortDirection === "asc" ? <ChevronUp className="ml-1 inline h-4 w-4" /> : <ChevronDown className="ml-1 inline h-4 w-4" />;
    };
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-h-[90vh] max-w-6xl overflow-y-auto">
                ...
                <div className="space-y-6">
                    {/* Batch Details Form */}
                    <form onSubmit={handleUpdateBatchDetails} className="space-y-4 rounded-lg border bg-slate-50 p-4">
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <Input value={data.tracking_number} onChange={(e) => setData("tracking_number", e.target.value)} className={errors.tracking_number ? "border-red-500" : ""} />
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                <label className="mb-1 block text-sm font-medium">Shipped Date</label>
                                <Input type="date" value={data.shipped_date} onChange={(e) => setData("shipped_date", e.target.value)} className={errors.shipped_date ? "border-red-500" : ""} />
                                {errors.shipped_date && <p className="mt-1 text-sm text-red-500">{errors.shipped_date}</p>}
                            </div>
                            <div>
                                <label className="mb-1 block text-sm font-medium">Est. Delivery</label>
                                <Input
                                    type="date"
                                ...
                                />
                                {errors.estimated_delivery && <p className="mt-1 text-sm text-red-500">{errors.estimated_delivery}</p>}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            ...
                            <div>
                                <label className="mb-1 block text-sm font-medium">Actual Delivery</label>
                                <Input
                                    type="date"
                                ...
                                />
                                {errors.actual_delivery && <p className="mt-1 text-sm text-red-500">{errors.actual_delivery}</p>}
                            </div>
                        </div>

                        <div>
                            <label className="mb-1 block text-sm font-medium">Notes</label>
                            <Textarea value={data.notes} onChange={(e) => setData("notes", e.target.value)} rows={3} className={errors.notes ? "border-red-500" : ""} />
                    ...
                    <div className="flex items-center justify-between">
                        <h4 className="font-medium">Watches in this batch ({batch.watches.length})</h4>
                        <Button onClick={() => onAddWatchToBatch(batch.routekey)} className="flex items-center gap-1">
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="cursor-pointer hover:bg-slate-100" onClick={() => onBatchWatchSort("name")}>
                                        Name {getSortIcon("name")}
                                    </TableHead>
                                    <TableHead className="cursor-pointer hover:bg-slate-100" onClick={() => onBatchWatchSort("sku")}>
                                        SKU {getSortIcon("sku")}
                                    </TableHead>
                                    <TableHead className="cursor-pointer hover:bg-slate-100" onClick={() => onBatchWatchSort("brand")}>
                            ...
                            <TableBody>
                                {getSortedBatchWatches(batch.watches).map((watch) => {
                                    const fullWatch = availableWatches.find((w) => w.id === Number(watch.originalId || watch.id));

                                    return (
                                        <TableRow key={`${batch.id}-${watch.id}`}>
                                            <TableCell>
                                                <img src={watch.image} alt={watch.name} className="h-12 w-12 rounded object-cover" />
                                            </TableCell>
                                            <TableCell className="font-medium">{watch.name}</TableCell>
                                            <TableCell>{watch.sku}</TableCell>
                                            <TableCell>{watch.brand}</TableCell>
                                            <TableCell>
                                                <Badge className={Status.toColorClass(fullWatch?.status || "Draft")}>{fullWatch?.status || "Unknown"}</Badge>
                                            </TableCell>
                                            <TableCell>{fullWatch?.location || batch.destination}</TableCell>
                                            <TableCell>
                                                <Button variant="outline" size="sm" onClick={() => onWatchRemove(batch.routekey, watch.routekey)} className="text-red-600 hover:text-red-700">
                                                    <X className="h-3 w-3" />
                                                </Button>
                                            </TableCell>
                                        </TableRow>
