...
const BatchManagement = () => {
    ...
    const [selectedBatchForWatch, setSelectedBatchForWatch] = useState<
        string | null
    >(null);
    ...
    const [batchWatchSortField, setBatchWatchSortField] =
        useState<string>("name");
    const [batchWatchSortDirection, setBatchWatchSortDirection] = useState<
        "asc" | "desc"
    >("asc");

    // Sorting states for add watch modal
    const [addWatchSortField, setAddWatchSortField] = useState<string>("name");
    const [addWatchSortDirection, setAddWatchSortDirection] = useState<
        "asc" | "desc"
    >("asc");
    ...
    const filteredAndSortedAvailableWatches = availableWatches
        .filter((watch) => {
            ...
            const matchesStatus =
                ...
                watch.status === watchStatusFilter;
    ...
    const handleBatchWatchSort = (field: string) => {
        if (batchWatchSortField === field) {
            setBatchWatchSortDirection(
                batchWatchSortDirection === "asc" ? "desc" : "asc",
            );
        } else {
    ...
    const handleAddWatchSort = (field: string) => {
        if (addWatchSortField === field) {
            setAddWatchSortDirection(
                addWatchSortDirection === "asc" ? "desc" : "asc",
            );
        } else {
    ...
    const handleSelectAllWatches = (checked: boolean) => {
        if (checked) {
            setSelectedWatchesToAdd(
                filteredAndSortedAvailableWatches.map((w) => w.id),
            );
        } else {
            setSelectedWatchesToAdd([]);
        }
    ...
    const handleSelectWatch = (watchId: string, checked: boolean) => {
        if (checked) {
            ...
            setSelectedWatchesToAdd(
                selectedWatchesToAdd.filter((id) => id !== watchId),
            );
        }
    };
        ...
        for (const batch of batches) {
            ...
            if (watch) {
                ...
                foundWatch = {
                    ...
                    images: watch.image
                        ? [{ id: "1", url: watch.image, useForAI: false }]
                        : [],
                };
                break;
            }
    ...
    const handleAddSelectedWatchesToBatch = () => {
        ...
        const watchesToAdd = availableWatches
            .filter((w) => selectedWatchesToAdd.includes(w.id))
            .map((watch) => ({
                id: watch.id,
                name: watch.name,
                sku: watch.sku,
    ...
    const currentEditingBatch = editingBatch
        ? batches.find((b) => b.id === editingBatch)
        : null;
            ...
            <div className="p-8">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        <h4 className="mb-3 font-medium">
                                            Watches in this batch (
                                            {batch.watches.length})
                                        </h4>
                                        {viewMode === "list" ? (
                    ...
                    <div className="py-12 text-center">
                        ...
                        <p className="text-slate-600">
                            {searchTerm || statusFilter !== "all"
                                ...
                                : "Create your first batch to start tracking shipments"}
                        </p>
                    </div>
                )}
                ...
                >
                    <DialogContent className="max-h-[90vh] max-w-6xl overflow-y-auto">
                        <DialogHeader>
                            <DialogTitle>
                                Edit Batch: {currentEditingBatch?.name}
                            </DialogTitle>
                        </DialogHeader>
                        {currentEditingBatch && (
                            <div className="space-y-6">
                                ...
                                <div className="space-y-4 rounded-lg border bg-slate-50 p-4">
                                    ...
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                        ...
                                        <div>
                                            ...
                                            <Input
                                                value={
                                                    editingBatchData.trackingNumber ||
                                                    ""
                                                }
                                                onChange={(e) =>
                                                    setEditingBatchData({
                                    ...
                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                        ...
                                        <div>
                                            ...
                                            <Input
                                                type="date"
                                                value={
                                                    editingBatchData.estimatedDelivery ||
                                                    ""
                                                }
                                                onChange={(e) =>
                                                    setEditingBatchData({
                                ...
                                <div className="flex items-center justify-between">
                                    <h4 className="font-medium">
                                        Watches in this batch (
                                        {currentEditingBatch.watches.length})
                                    </h4>
                                    <Button
                                        onClick={() =>
                                            openAddWatchModal(
                                                currentEditingBatch.id,
                                            )
                                        }
                                        className="flex items-center gap-1"
                                    >
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {getSortedBatchWatches(
                                                currentEditingBatch.watches,
                                            ).map((watch) => {
                                                const fullWatch =
                                                    availableWatches.find(
                                                        (w) =>
                                                            w.id === watch.id,
                                                    );
                                                return (
                                                    <TableRow key={watch.id}>
                                                        ...
                                                        <TableCell>
                                                            {fullWatch?.location ||
                                                                currentEditingBatch.destination}
                                                        </TableCell>
                                                        <TableCell>
                                                            <Button
                                                                variant="outline"
                ...
                <Dialog
                    open={isAddWatchModalOpen}
                    onOpenChange={setIsAddWatchModalOpen}
                >
                    <DialogContent className="max-h-[90vh] max-w-6xl overflow-y-auto">
                        <DialogHeader>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-4">
                                <div className="relative flex-1">
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) =>
                                            setWatchSearchTerm(e.target.value)
                                        }
                                        className="pl-10"
                                    />
                                </div>
                                <Select
                                    ...
                                    onValueChange={setWatchStatusFilter}
                                >
                                    <SelectTrigger className="w-48">
                                        <SelectValue placeholder="Filter by status" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        ...
                                        <SelectItem value="Platform Review">
                                            Platform Review
                                        </SelectItem>
                                        <SelectItem value="Ready for listing">
                                            Ready for listing
                                        </SelectItem>
                                        ...
                                        <SelectItem value="Defect/Problem">
                                            Defect/Problem
                                        </SelectItem>
                            ...
                            <div className="rounded-lg border">
                                <Table>
                                                ...
                                                <Checkbox
                                                    checked={
                                                        selectedWatchesToAdd.length ===
                                                            filteredAndSortedAvailableWatches.length &&
                                                        filteredAndSortedAvailableWatches.length >
                                                            0
                                                    }
                                                    onCheckedChange={
                                    ...
                                    <TableBody>
                                        {filteredAndSortedAvailableWatches.map(
                                            (watch) => (
                                                <TableRow key={watch.id}>
                                                    <TableCell>
                                                        <Checkbox
                                                            checked={selectedWatchesToAdd.includes(
                                                                watch.id,
                                                            )}
                                                            onCheckedChange={(
                                                                checked,
