...
const AgentWatches = () => {
  ...
  const availablePaymentIds = [
    ...
    'PAY-036', 'PAY-037', 'PAY-038', 'PAY-039', 'PAY-040',
    'PAY-041', 'PAY-042', 'PAY-043', 'PAY-044', 'PAY-045',
    'PAY-046', 'PAY-047', 'PAY-048', 'PAY-049', 'PAY-050'
  ];
  ...
  const availableSellers = [
    'John Smith', 'Maria Garcia', 'Robert Johnson', 'Emma Wilson',
    'David Brown', 'Lisa Davis', 'Michael Chen', 'Sarah Thompson',
    'James Martinez', 'Anna Rodriguez', 'Thomas Lee', 'Jennifer White'
  ];

  // Add brands state for the BrandSelector
  const [brands, setBrands] = useState<string[]>([
    'Rolex', 'Omega', 'Tudor', 'Patek Philippe', 'Audemars Piguet', 'Cartier',
    'Jaeger-LeCoultre', 'Vacheron Constantin', 'IWC', 'Breitling', 'Panerai',
    'TAG Heuer', 'Hublot', 'Zenith', 'Longines', 'Seiko', 'Casio', 'Citizen',
    'Tissot', 'Hamilton', 'Frederique Constant', 'Montblanc', 'Oris'
  ]);
  ...
  const totalValue = filteredWatches.reduce((sum, watch) => sum + watch.price, 0);
  ...
  const handleSelectAll = (checked: boolean) => {
    setSelectedWatches(checked ? filteredWatches.map(watch => watch.id) : []);
  };
    ...
    <Layout>
      ...
      <div className="p-6 space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Agent Watches</h1>
            <p className="text-slate-600 mt-1">Manage agent watch inventory</p>
          </div>
          <div className="flex gap-2">
            <Button onClick={handleAddWatch} className="flex items-center gap-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="text-sm font-medium text-slate-700 mb-2 block">Seller</label>
                <Select value={sellerFilter} onValueChange={setSellerFilter}>
                  ...
                  <SelectContent>
                    ...
                    {uniqueSellers.filter(seller => seller && seller.trim() !== '').map((seller) => (
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-slate-700">
                  {selectedWatches.length} watch{selectedWatches.length !== 1 ? 'es' : ''} selected
                </span>
                <div className="flex gap-2">
                  ...
                  <Popover open={assignOpen} onOpenChange={setAssignOpen}>
                    ...
                    <PopoverContent className="w-64 p-0" align="start">
                      <Command>
                        ...
                        />
                        <CommandList>
                          <CommandEmpty>No Payment ID found.</CommandEmpty>
                          <CommandGroup>
                            {filteredPaymentIds.map((paymentId) => (
                              <CommandItem
                                key={paymentId}
                                value={paymentId}
                                onSelect={() => handlePaymentIdAssign(paymentId)}
                              >
                                {paymentId}
                              </CommandItem>
                            ))}
          ...
          <CardContent>
            <div className="grid grid-cols-5 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-slate-900">{totalWatches}</div>
                <div className="text-sm text-slate-600">Total Watches</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-red-600">
                  {filteredWatches.filter(w => w.status === 'Unpaid').length}
                </div>
                <div className="text-sm text-slate-600">Unpaid</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-yellow-600">
                  {filteredWatches.filter(w => w.status === 'Paid (not received)').length}
                </div>
                <div className="text-sm text-slate-600">Paid (not received)</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">
                  {filteredWatches.filter(w => w.status === 'Paid (received)').length}
                </div>
                <div className="text-sm text-slate-600">Paid (received)</div>
              </div>
              <div className="text-center">
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="cursor-pointer hover:bg-slate-50" onClick={() => handleSort('sku')}>
                      <div className="flex items-center">
                        SKU
                        {getSortIcon('sku')}
                      </div>
                    </TableHead>
                    <TableHead className="cursor-pointer hover:bg-slate-50" onClick={() => handleSort('brand')}>
                      <div className="flex items-center">
                        Brand
                        {getSortIcon('brand')}
                      </div>
                    </TableHead>
                    <TableHead className="cursor-pointer hover:bg-slate-50" onClick={() => handleSort('seller')}>
                      <div className="flex items-center">
                        Seller
                        {getSortIcon('seller')}
                      </div>
                    ...
                    <TableHead className="w-32 cursor-pointer hover:bg-slate-50" onClick={() => handleSort('price')}>
                      <div className="flex items-center">
                        Price
                        {getSortIcon('price')}
                      </div>
                    </TableHead>
                    <TableHead className="cursor-pointer hover:bg-slate-50" onClick={() => handleSort('paymentId')}>
                      <div className="flex items-center">
                        Payment ID
                    ...
                    <TableRow key={watch.id}>
                      ...
                      <TableCell>
                        <div className="w-12 h-12 rounded-lg overflow-hidden bg-slate-100">
                          <img
                            src={watch.images.length > 0 ? watch.images[0].url : DEFAULT_IMAGE}
                            alt={watch.name}
                      ...
                      <TableCell>
                        <div className="text-sm text-slate-600">{watch.sku}</div>
                      </TableCell>
                      ...
                      <TableCell>
                        <div className="text-sm text-slate-900">
                          ₫{(watch.price * 25000).toLocaleString()}
                        </div>
                      ...
                      <TableCell>
                        <div className="text-sm text-slate-600">{watch.paymentId || '-'}</div>
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          <Button variant="ghost" size="sm" onClick={() => handleEditWatch(watch)}>
              ...
              <Card key={watch.id} className="overflow-hidden">
                <div className="aspect-square relative">
                  <img
                    ...
                    className="w-full h-full object-cover"
                  />
                  <Badge className={`absolute top-2 right-2 ${getStatusColor(watch.status)}`}>
                    {watch.status}
                  </Badge>
                ...
                <CardContent className="p-4">
                  <h3 className="font-semibold text-lg mb-1 truncate">{watch.name}</h3>
                  <div className="text-sm text-slate-600 mb-2">{watch.sku}</div>
                  <div className="space-y-1 text-sm text-slate-600">
                    <div><span className="font-medium">Brand:</span> {watch.brand}</div>
                    <div><span className="font-medium">Seller:</span> {watch.seller}</div>
                    <div className="text-xs text-slate-500 mb-2 truncate">{watch.notes}</div>
                    <div className="mt-2">
                      <div className="text-sm text-slate-900">
                        ₫{(watch.price * 25000).toLocaleString()}
                      </div>
        ...
        {filteredWatches.length === 0 && (
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <p className="text-slate-600">No watches match your current filters</p>
            </CardContent>
          </Card>
        )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.agent} onValueChange={(value) => setFormData(prev => ({ ...prev, agent: value }))}>
                    ...
                    <SelectContent>
                      {availableAgents.filter(agent => agent && agent.trim() !== '').map(agent => (
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center space-x-2">
                    <Input
                      ...
                      value={formData.sku}
                      readOnly
                      className="flex-1 bg-slate-50 text-slate-600 cursor-not-allowed"
                ...
                <div className="space-y-2">
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      ...
                      >
                        {formData.paymentId || "Select payment ID"}
                    ...
                    <PopoverContent className="w-full p-0">
                      <Command>
                        ...
                        <CommandList>
                          <CommandEmpty>No payment ID found.</CommandEmpty>
                          <CommandGroup>
                            {availablePaymentIds.map((paymentId) => (
                              <CommandItem
                                key={paymentId}
                                value={paymentId}
                                onSelect={() => setFormData(prev => ({ ...prev, paymentId }))}
                              >
                                {paymentId}
                              </CommandItem>
                            ))}
            ...
            <div className="space-y-4">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      className={cn(
                        ...
                        !newPaymentData.date && "text-muted-foreground"
                      )}
                    >
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {newPaymentData.date ? format(newPaymentData.date, "PPP") : <span>Pick a date</span>}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      ...
                      onSelect={(date) => setNewPaymentData(prev => ({ ...prev, date: date || new Date() }))}
                      initialFocus
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => setNewPaymentData(prev => ({ ...prev, paymentDescription: e.target.value }))}
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) => setNewPaymentData(prev => ({ ...prev, documentation: e.target.value }))}
