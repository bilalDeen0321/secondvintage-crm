...
type SortField = 'watchName' | 'brand' | 'sku' | 'acquisitionCost' | 'salePrice' | 'profit' | 'profitMargin' | 'saleDate' | 'platform' | 'buyer' | 'status';
...
const SalesHistory = () => {
      ...
      if (timeRange !== 'All Time') {
        ...
        switch (timeRange) {
          case 'Last 7 days':
            ...
            break;
          case 'Last 30 days':
            matchesTime = saleDate >= new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
            break;
          case 'Last 3 months':
            matchesTime = saleDate >= new Date(now.getTime() - 90 * 24 * 60 * 60 * 1000);
            break;
          case 'Last 6 months':
            matchesTime = saleDate >= new Date(now.getTime() - 180 * 24 * 60 * 60 * 1000);
            break;
          case 'This year':
  ...
  }, [sales, searchTerm, platformFilter, statusFilter, sortField, sortDirection, timeRange]);
    ...
    <Layout>
      <div className="p-8">
        ...
        <div className="mb-8 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">Sales History & Statistics</h1>
            <p className="text-slate-600 mt-1">Track your sales performance and revenue</p>
          </div>
          <Button>
        ...
        <Card className="mb-8">
          <CardHeader>
            <CardTitle className="text-lg">Time Range Filter</CardTitle>
            <CardDescription>Filter sales by time period</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Stacked revenue and profit trends over time</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Multi-line chart showing profit trends for each platform</CardDescription>
            </CardHeader>
            <CardContent>
              <ChartContainer config={profitChartConfig} className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={profitPerPlatformData}>
                    ...
                    <YAxis tickFormatter={(value) => `€${value.toLocaleString()}`} />
                    <ChartTooltip content={<ChartTooltipContent />} />
                    <Line type="monotone" dataKey="Chrono24" stroke="var(--color-Chrono24)" strokeWidth={2} />
                    <Line type="monotone" dataKey="eBay" stroke="var(--color-eBay)" strokeWidth={2} />
                    <Line type="monotone" dataKey="Catawiki" stroke="var(--color-Catawiki)" strokeWidth={2} />
                    <Line type="monotone" dataKey="Tradera" stroke="var(--color-Tradera)" strokeWidth={2} />
                  </LineChart>
                </ResponsiveContainer>
              </ChartContainer>
            </CardContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                  ...
                  <div key={platform.platform} className="flex items-center gap-2 p-2 rounded-lg border">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-slate-500">
                        {platform.sales} sales • €{platform.profit.toLocaleString()} profit • {platform.percentage}%
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Number of watches sold monthly</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
        ...
        <Card>
          <CardContent className="p-0">
            <Table>
              <TableHeader>
                <TableRow>
                  <SortableHeader field="watchName">Watch Name</SortableHeader>
              ...
              <TableBody>
                {sortedAndFilteredSales.map((sale) => (
                  <TableRow key={sale.id}>
                    <TableCell className="font-medium">{sale.watchName}</TableCell>
                    <TableCell>{sale.brand}</TableCell>
                    <TableCell className="text-slate-600">{sale.sku}</TableCell>
                    <TableCell>€{sale.acquisitionCost.toLocaleString()}</TableCell>
                    <TableCell className="font-semibold">€{sale.salePrice.toLocaleString()}</TableCell>
                    <TableCell className="font-semibold text-green-600">€{sale.profit.toLocaleString()}</TableCell>
